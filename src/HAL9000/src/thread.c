...
/*
static int functieReturn()
{
    return m_threadSystemData.numarThreads;
}
*/
...
{
    m_threadSystemData.numarThreads = 0;
    STATUS status;
    PPCPU pCpu;
...
#pragma warning(suppress:4152)
    ...
    pThread->parentId = ((THREAD*)__readmsr(IA32_FS_BASE_MSR))->Id;
    *Thread = pThread;
    m_threadSystemData.numarThreads++;
    return status;
}
    ...
    )
{
    ...
    __try
    {
        ...
        LOG("~~~Thread created %s %d~~~", pThread->Name, pThread->Id);
    }
    __finally
    {
        if (!SUCCEEDED(status))
