    ...
    )
{
    ...
    UNREFERENCED_PARAMETER(Context);
    int numar = *((int*)Context);
    pCpu = GetCurrentPcpu();

    LOG(" Hello   from  CPU  0x %02x [0x%02x] %d\n", pCpu->ApicId, pCpu->LogicalApicId,numar);
    ...
    )
{
    ...
    LOG(" Hello TEST  from  CPU  0x %02x [0x%02x]\n", pCpu->ApicId, pCpu->LogicalApicId);
    ...
    )
{
    ...
    LOG(" Hello SEXY  from  CPU  0x %02x [0x%02x]\n", pCpu->ApicId, pCpu->LogicalApicId);
...
#pragma warning(suppress: 28039)
    ...
    */
    ...
    LOG(" Hello   from  CPU  0x %02x [0x%02x]\n", pCpu->ApicId, pCpu->LogicalApicId);
    //LOG(" Hello   from  CPU  0x %02x [0x%02x]\n", pCpu->ApicId, pCpu->LogicalApicId);
    BYTE var = ((BYTE) SmpGetNumberOfActiveCpus()) - 1;
    SMP_DESTINATION dest = { var };
    status = SmpSendGenericIpiEx(_CmdIpiCmd1, NULL, NULL, NULL, FALSE, SmpIpiSendToCpu, dest);
    if (!SUCCEEDED(status))
    {
        LOG_FUNC_ERROR(" SmpSendGenericIpi ", status);
    }

    // dest.group.affinity=0b ceva; ii posibil sa activezi alea impare si sa trimiti ca si grup

    int nr = SmpGetNumberOfActiveCpus();
    for (int i = 0; i < nr; i++)
    {
        if (i % 2 != 0)
        {
            var = ((BYTE)i) ;
            SMP_DESTINATION dest1 = { var };
            status = SmpSendGenericIpiEx(_CmdIpiCmd2, NULL, NULL, NULL, FALSE, SmpIpiSendToCpu, dest1);
            if (!SUCCEEDED(status))
            {
                LOG_FUNC_ERROR(" SmpSendGenericIpi ", status);
            }
    ...
    /*
    QWORD* tickCount = ExAllocatePoolWithTag();
    *tickCount = RtcGetTickCount();
    * */
    QWORD x = RtcGetTickCount();
    PVOID numar= &x;
    status = SmpSendGenericIpiEx(_CmdIpiCmd3, numar, NULL, NULL, FALSE, SmpIpiSendToGroup, dest);
    if (!SUCCEEDED(status))
    {
        LOG_FUNC_ERROR(" SmpSendGenericIpi ", status);
    }
